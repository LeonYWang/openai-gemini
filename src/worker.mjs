...
function initializeApiKeys(env) {
  if (apiKeys.length === 0) {
    const keys_str = env.GEMINI_API_KEYS;
    if (keys_str) {
      apiKeys = keys_str.split(',').map(k => k.trim()).filter(Boolean);
    }
  }
  if (apiKeys.length === 0) {
...
export default {
  // Add `env` as the second argument to access environment variables
  async fetch(request, env) {
    // Initialize API keys from environment variables on the first run
    try {
        initializeApiKeys(env);
    } catch (err) {
    ...
    try {
      // --- MODIFICATION: Use rotated API key instead of the one from header ---
      const apiKey = getNextApiKey();
      
      // Original logic for checking Authorization header is removed.
...
// ... a lot of code (The rest of your code remains unchanged)
